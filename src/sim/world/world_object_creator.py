...
class WorldObjectCreator:
    ...
    def create_lim_world(self):
        ...
        # [냉장고 내부/주변 음식 및 자원]
        water_bottle = ItemObject(name="생수병", detail="마시다 남은 투명한 생수병. 차가운 한기가 서려 있다.", parent=kitchen)
        water_bottle.position.x = 2.0  # 냉장고(2,2) 내부 혹은 바로 옆 배치
        water_bottle.position.y = 3.0
        objects.append(water_bottle)

        canned_food = ItemObject(name="통조림", detail="유통기한이 정체된 먼지 쌓인 통조림. 영양을 보충할 수 있을 것 같다.", parent=kitchen)
        canned_food.position.x = 3.0
        canned_food.position.y = 2.0
        objects.append(canned_food)

        # [식탁 및 싱크대 위 음식 오브젝트]
        stale_bread = ItemObject(name="딱딱한 빵", detail="수분이 완전히 날아가 딱딱하게 굳어버린 식빵 한 조각.", parent=kitchen)
        stale_bread.position.x = 5.0  # 식탁(5,5) 위에 놓여 있음
        stale_bread.position.y = 5.0
        objects.append(stale_bread)

        rotten_apple = ItemObject(name="썩은 사과", detail="검게 진물러 터진 사과. 마치 내면의 썩어가는 정서와 닮아 있다.", parent=kitchen)
        rotten_apple.position.x = 2.0  # 싱크대(2,6) 구석에 방치됨
        rotten_apple.position.y = 7.0
        objects.append(rotten_apple)
