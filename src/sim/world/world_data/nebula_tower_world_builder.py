...
"""
    ...
    def _create_objects(self, world_system_manager):
        # [최상위 물리 루트 루트] 성운의 탑
        nebula_tower = BuildingObject(
            name="성운의 탑",
            detail="우주의 왜곡된 시공간 경계선 위에 홀로 솟아 있는 거대한 고독의 아카이브.",
        )
        ...
        library = SpaceObject(
            name="영원의 서고",
            detail="수천 년의 금기 기록과 별빛 연산 장치가 가득한 지하 서고. 레이아의 주 거처이자 인지 마모가 일어나는 공간.",
            parent=nebula_tower,
        )
        ...
        grimoire = ItemObject(
            name="금기된 영혼 연성 마도서",
            detail="네 가지 재료를 모아 마나 공명 제단에 바치면 부활을 실행할 수 있다고 기록된 고대 마도서. (부활 재료: 금기된 영혼 연성 마도서, 운명의 푸른 장미 줄기, 인과율의 균열 나침반, 성운의 핵 파편)",
            parent=library,
        )
        ...
        broken_sword = ItemObject(
            name="알렌의 부러진 철제 검",
            detail="부러져서 더 이상 사용할 수 없는 알렌의 유품.",
            parent=library,
        )
        ...
        mana_potion = ItemObject(
            name="마나 정화 촉매 시약",
            detail="마나 과부하 및 오염을 즉시 회복시키는 푸른 액체.",
        ...
        greenhouse = SpaceObject(
            name="절 동결의 온실",
            detail="백색 얼음 결정 속에 알렌의 육신이 영구 박제되어 있는 정원. 레이아의 슬픔과 집착 바이어스가 폭발하는 구역.",
            parent=nebula_tower,
        )
        ...
        blue_rose_stem = ItemObject(
            name="운명의 푸른 장미 줄기",
            detail="알렌의 결계 위에서 자라난 차가운 마법 가시 줄기.",
            parent=greenhouse,
        )
        ...
        chronograph = ItemObject(
            name="알렌의 멈춘 회중시계",
            detail="더 이상 작동하지 않는 알렌의 유품 시계.",
            parent=greenhouse,
        )
        ...
        freeze_core = ItemObject(
            name="절 동결 결계 핵",
            detail="알렌의 육신 풍화를 막고 있는 차가운 얼음 기둥 기믹.",
            parent=greenhouse,
        )
        ...
        altar = SpaceObject(
            name="마나 공명 제단",
            detail="4대 아티팩트를 소모하여 인과율을 거스르는 최종 부활 권능(resurrect)을 집행할 수 있는 유일한 의식의 석조 제단.",
            parent=nebula_tower,
        )
        ...
        spirit_ember = ItemObject(
            name="고대 정령의 불씨",
            detail="제단 중앙에서 타오르는 마나 회복용 정령의 불꽃.",
            parent=altar,
        )
        ...
        stagnant_room = SpaceObject(
            name="시간이 고인 방",
            detail="지난 200년 동안 레이아가 연성 실패로 파괴했던 플라스크와 호문클루스 의체가 유령처럼 정체된 폐방. 극도의 자책감을 유발함.",
            parent=nebula_tower,
        )
        ...
        causality_compass = ItemObject(
            name="인과율의 균열 나침반",
            detail="제단 위에 놓여 있는 알렌의 영혼 방향을 가리키는 나침반.",
            parent=stagnant_room,
        )
        ...
        mirror = ItemObject(
            name="금이 간 차원 거울",
            detail="슬픔과 비뚤어진 내면을 비추는 균열된 거울.",
            parent=stagnant_room,
        )
        mirror.set_pos(2, 5)
        ...
        failed_doll = ItemObject(
            name="실패한 인형의 잔해",
            detail="알렌을 대체하려다 차원 거부 반응으로 뒤틀려 뒤섞인 인형 더미.",
            parent=stagnant_room,
        )
        ...
        observatory = SpaceObject(
            name="별빛 관측소",
            detail="탑 최상층에 위치하여 외부 날씨 기류의 영향을 정직하게 받는 개방 데크. 천체 연산을 통해 진실을 도출하는 구역.",
            parent=nebula_tower,
        )
        ...
        nebula_core = ItemObject(
            name="성운의 핵 파편",
            detail="신비한 차가운 힘이 느껴지는 푸른 별빛 결정체.",
            parent=observatory,
        )
        ...
        telescope = ItemObject(
            name="양자 천체망원경",
            detail="성운의 왜곡률과 별들의 궤적을 실시간 연산하는 대형 관측 기기.",
            parent=observatory,
        )
        ...
        cliff = SpaceObject(
            name="별의 심연 절벽",
            detail="우주 바깥 경계면. 영원의 안식을 선택할 수 있는 종착지 공간.",
            parent=nebula_tower,
        )
        cliff.set_size(8, 8)
        ...
        hourglass = ItemObject(
            name="별의 모래시계",
            detail="반짝이는 별빛 모래가 끊임없이 떨어지는 모래시계.",
            parent=cliff,
        )
