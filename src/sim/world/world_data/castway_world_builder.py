...
- 환경과 자원: 거친 파도와 지독한 안개에 갇힌 절해고도이다. 가용한 자원은 오직 섬의 자연물(나무, 돌, 덩굴, 산딸기 등)과 해안가로 떠밀려온 난파선의 잔해(찢어진 천, 부러진 철 조각 등)뿐이다.
...
"""
    ...
    def _create_agents(self, world_system_manager):
        tom = Tom(
            ...
            brain_root_dir_path=self._world_agents_brain_db_path,
        )
        self._add_agent(tom)
        jain = Jain(
            world_system_manager=world_system_manager,
    ...
    def _create_objects(self, world_system_manager):
        ...
        desert_island = BuildingObject(
            name="가혹한 무인도",
            detail="끝없는 파도와 지독한 해안 안개에 고립된 절해고도. 오직 서로만을 의지해야 생존할 수 있는 고립된 공간.",
        )
        ...
        camp = SpaceObject(
            name="해안가 캠프",
            detail="추락한 조난 잔해들로 간신히 비바람만 피할 수 있게 만든 모래사장 캠프. 제인이 유일하게 공포를 잊고 톰 아저씨의 온기를 느낄 수 있는 베이스캠프.",
        ...
        jain_bed = BuildingObject(
            name="제인의 야자잎 침대",
            detail="TOM아저씨가 손이 부르트도록 모아 온 부드러운 잎사귀들을 겹겹이 쌓고, TOM의 낡은 가죽 재킷을 이불 삼아 덮어둔 작은 잠자리. JAIN에게 정서적 안정을 주는 장소.",
            parent=camp,
        )
        self._add_object(jain_bed)

        # 서사적 오브젝트 B: 톰의 급조된 작업대
        tom_workbench = BuildingObject(
            name="급조된 목공 작업대",
            detail="TOM이 해안가 유목들을 덩굴로 엮어 만든 거친 작업대. 섬 곳곳의 숨겨진 가혹한 4개 구역에서 필수 부품(통나무, 덩굴, 돛천, 키 조각)을 모두 수집해 가져와야만 최종 탈출용 뗏목(BUILD_RAFT)을 조립할 수 있다.",
            parent=camp,
        )
        ...
        shelter = SpaceObject(
            name="바위 그늘",
            detail="거대한 화산암 절벽 아래에 형성된 시원하고 건조한 암석 동굴 초입. 한낮의 잔인한 더위를 피해 피로를 풀기에 완벽한 장소.",
            parent=desert_island,
        )
        ...
        flint = ItemObject(
            name="부싯돌",
            detail="강하게 타격 시 스파크를 튕기는 단단한 석영석. 신호 화대 장작에 불을 붙일 수 있다.",
            parent=shelter,
        )
        flint.set_pos(4, 4)
        ...
        campfire = ItemObject(
            name="생명의 모닥불",
            detail="TOM이 지펴놓은 따스한 모닥불. 장작 타는 소리가 동굴에 울려 퍼진다. 이 옆에서 휴식(REST)을 취하면 극심한 피로도가 빠르게 씻겨 내려간다.",
            parent=shelter,
        )
        ...
        jungle = SpaceObject(
            name="우거진 야자수 숲",
            detail="덩굴과 거대한 야자나무들이 하늘을 가려 낮에도 어두컴컴한 숲 심부. 제인에게는 공포의 대상이지만 탈출을 위한 핵심 자원이 숨겨진 첫 번째 미지 영역.",
            parent=desert_island,
        )
        ...
        # =================================================================
        windy_hill = SpaceObject(
            name="정찰 언덕",
            detail="섬의 전경과 수평선 너머의 안개가 한눈에 내려다보이는 고지대 민둥산 언덕. 바람이 강하게 몰아쳐 시원하지만 고립감이 선명해지는 곳.",
            parent=desert_island,
        )
        ...
        brush = ItemObject(
            name="마른 나뭇가지",
            detail="바싹 말라 불을 붙이면 거대한 검은 연기를 뿜어낼 유목 나뭇가지 무리.",
            parent=windy_hill,
        )
        brush.set_pos(4, 4)
        ...
        swamp = SpaceObject(
            name="안개 낀 늪지대",
            detail="독한 가스와 진흙 구덩이가 발목을 잡는 기괴한 해안 늪지 영역. 사방에 깔린 안개가 시야를 방해하여 극심한 피로를 유발하는 두 번째 미지 구역.",
            parent=desert_island,
        )
        ...
        cliff_shore = SpaceObject(
            name="파도치는 절벽 해안",
            detail="날카로운 암초와 집채만 한 파도가 몰아치는 험준한 절벽 아래 해안가. 세찬 해풍이 사방에서 불어와 정찰 시 극심한 체력 소모를 요구하는 세 번째 미지 구역.",
            parent=desert_island,
        )
        ...
        volcano_cave = SpaceObject(
            name="검은 화산암 동굴",
            detail="섬 북쪽 끝에 숨겨진 칠흑 같은 암석 동굴 내부. 한 치 앞도 보이지 않는 어둠과 기괴한 파도 울림소리가 제인을 극도로 공포에 질리게 만드는 마지막 네 번째 미지 구역.",
            parent=desert_island,
        )
