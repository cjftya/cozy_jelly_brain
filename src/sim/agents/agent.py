...
class Agent(AtomicObject):
    ...
    def __init__(
        ...
        self.engine = JellyEngine(
            self.name, self.world_system_manager, brain_root_dir_path
        )
    ...
    def tick(self, time_scale):
        ...
        self._update_environmental_debuff(
            day_cycle=day_cycle, weather_type=weather_type, time_scale=time_scale
        )
    ...
    def think_tick(self):
        ...
        if any(
            [
                event_type in self.think_event_queue.keys()
                for event_type in [ThinkEventType.FATIGUE, ThinkEventType.HUNGER]
            ]
        ):
            ...
            res = self.engine.think_event_normal(
                agent=self,
                external_event=combined_signal,
                available_tool_types=self.get_available_tool_types(),
            )
            return res
        ...
        if ThinkEventType.FIND_AGENT in self.think_event_queue.keys():
            ...
            self.think_event_queue.clear()
            res = self.engine.think_event_speak(
                user_input=think_event_message,
                agent=self,
        ...
        if ThinkEventType.FIND_ITEM in self.think_event_queue.keys():
            ...
            self.think_event_queue.clear()
            res = self.engine.think_event_detect_objects(
                agent=self,
                external_event=think_event_message,
        ...
        if ThinkEventType.SPEAK in self.think_event_queue.keys():
            ...
            available_agent = self.world_system_manager.agent_manager.get_agent_by_name(
                found_agent_name
            )
            ...
            res = self.engine.think_event_speak(
                ...
                agent=self,
                available_agents=[available_agent],
                from_scan=False,
                available_tool_types=self.get_available_tool_types(),
            )
            return res
    ...
    def scan(self, external_event):
        ...
        if action_roll < 0.5 and len(found_agents) > 0:
            if self.vital_state.fatigue < 80 and self.vital_state.health > 30:
                ...
                if ran_num >= 1.0:
                    ...
                    self.push_think_event(
                        ThinkEventType.FIND_AGENT, external_event + msg, found_agents
                    )
                    return
        ...
        if len(found_objects) > 0:
            ...
            self.push_think_event(
                ThinkEventType.FIND_ITEM, external_event + msg, found_objects
            )
            return
    ...
    def perceive_objects(self):
        world_objects = self.world_system_manager.object_manager.get_objects_by_type(
            ObjectType.ITEM
        )
