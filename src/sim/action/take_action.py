    ...
    def execute(self, *args):
        ...
        if len(args) != 2:
            self.world_system_manager.log_system_event(
                "skip function call: take, args length not 2"
            )
            return False
        ...
        if not agent:
            self.world_system_manager.log_system_event(
                "skip function call: take, agent null"
            )
            return False

        target_object = self.world_system_manager.object_manager.pop_object_by_id(
            object_id
        )
        if not target_object:
            self.world_system_manager.log_system_event(
                "skip function call: take, target object null"
            )
            return False
        ...
        self.world_system_manager.log_world_event(
            f"{agent.name}가 {target_object.name}을 획득."
        )
        return True
