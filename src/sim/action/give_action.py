    ...
    def execute(self, *args):
        ...
        if len(args) != 3:
            self.world_system_manager.log_system_event(
                "skip function call: give, args length not 3"
            )
            return False
        ...
        source_agent = self.world_system_manager.agent_manager.get_agent_by_name(
            source_agent_name
        )
        if not target_agent or not source_agent:
            self.world_system_manager.log_system_event(
                "skip function call: give, target or source agent null"
            )
            return False
        ...
        if not target_object:
            self.world_system_manager.log_system_event(
                "skip function call: give, target object null or not found in inventory"
            )
            return False
