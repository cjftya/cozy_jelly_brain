    ...
    def execute(self, *args):
        ...
        if len(args) != 3:
            self.world_system_manager.log_system_event(
                "skip function call: modify_mind, args length not 3"
            )
            return False
        ...
        if not target_agent:
            self.world_system_manager.log_system_event(
                "skip function call: modify_mind, target agent null"
            )
            return False
        ...
        try:
            ...
            self.world_system_manager.log_world_event(
                f"{target_agent_name}의 {key_name} -> {new_value}로 변경됨"
            )
            return True

        except Exception as e:
            self.world_system_manager.log_system_event(
                f"skip function call: modify_mind, error: {e}"
            )
            return False
