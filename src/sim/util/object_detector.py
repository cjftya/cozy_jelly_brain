...
class ObjectDetector:
    ...
    def _calculate_dynamic_range(self, matrix, vital_state):
        ...
        fatigue_factor = 1.0 - (
            min(fatigue, 100) / 100.0 * 0.5
        )  # 피로도 맥스 시 시야 50% 감소

        # 3. curiosity_indifference (낮을수록 호기심 많음 -> 범위 넓음)
        curiosity_indifference = (
            matrix.get("curiosity_indifference", 0.5) if matrix else 0.5
        )
        curiosity_factor = 1.3 - (
            curiosity_indifference * 0.5
        )  # 호기심이 많을수록 보너스

        # 최종 동적 시야 거리 계산
        dynamic_range = (
            self.base_range * fear_factor * fatigue_factor * curiosity_factor
        )
