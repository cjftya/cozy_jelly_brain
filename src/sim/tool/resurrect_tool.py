...
class ResurrectTool(BaseTool):
    ...
    def execute(self, params, agent, world_system_manager):
        ...
        if current_loc != "마나 공명 제단":
            agent.push_think_event(
                ThinkEventType.PLANNING,
                "'마나 공명 제단'이 아니라 부활을 실행할 수 없다.",
            )
            world_system_manager.log_world_event(
                f"{agent.name}가 제단이 아닌 곳에서 부활을 시도했으나 차가운 인과율의 침묵만이 있음."
            )
            return
        ...
        if not all(item in item_set for item in required_items):
            ...
            agent.push_think_event(
                ThinkEventType.PLANNING,
                f"{', '.join(missing_items)}를 찾고 있으나, 인벤토리에 존재하지 않아 찾을 수 없음.",
            )
            world_system_manager.log_world_event(
                f"{agent.name}가 부활을 위해 {', '.join(missing_items)}를 찾고 있으나, 인벤토리에 존재하지 않아 찾을 수 없음."
            )
            return
        ...
        try:
            ...
            world_system_manager.log_system_event(
                "아스트리 레이아의 JellyBrain 시냅스 메모리 내 '알렌' 엔티티 및 유대 트리플렛 영구 삭제 완료."
            )
        except Exception as e:
            world_system_manager.log_system_event(
                f"메모리 삭제 중 예외 발생 (헤드리스 무결성 유지): {e}"
            )
        ...
        agent.tool_manager.add_all_available_tool_types([ToolType.SPEAK, ToolType.GIVE])
        ...
        if allen:
            ...
            allen.push_think_event(
                ThinkEventType.PLANNING, "여기는.. 어디지? 난 분명 그때..."
            )
            agent.push_think_event(
                ThinkEventType.SPEAK,
                "알렌, 드디어 널 만날 수 있어, 정말 긴 시간 이었어...",
                allen.name,
            )
            world_system_manager.log_world_event(
            ...
            world_system_manager.log_system_event(
                "오류: 월드 내에서 '알렌' 에이전트 인스턴스를 찾을 수 없음."
            )
